...
data = pd.read_json('F:/somebody gonna get hurt/reference projects/train.json')
test = pd.read_json('F:/somebody gonna get hurt/reference projects/test.json')

print('Training data shape: {}'.format(data.shape))
print('Test data shape: {}'.format(test.shape))

type(data.loc[1, "ingredients"])
# Target variable 
target = data.cuisine
type(target)
target.describe()
