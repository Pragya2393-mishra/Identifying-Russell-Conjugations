...
f1=np.mean(f1) #0.680154723894253
accuracy = np.mean(accuracy) #0.8861054219110747
precision = np.mean(precision) #0.6563212947872049
recall= np.mean(recall) #0.7258420780182498
...
test1 = pd.read_csv('F:/somebody gonna get hurt/Github/Model/Test files for try17/test8.csv')
l =[]
...
accuracy= accuracy_score(test1['y'],y_pred) #0.9666666666666667
f1=f1_score(test1['y'],y_pred) #0.967741935483871
precision= precision_score(test1['y'],y_pred) #1.0
recall=recall_score(test1['y'],y_pred) #0.9375
test1['y-pred']=y_pred
test1.to_csv('F:/somebody gonna get hurt/Github/Model/Results/Results_Test8_data17.csv')