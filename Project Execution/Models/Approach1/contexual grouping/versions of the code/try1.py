...
X_train['res_vector'] = X_train.x.apply(resultant_vector)
X_test['res_vector'] = X_test.x.apply(resultant_vector)
...
result = sm.Logit(y, X).fit().summary()
coeff = sm.Logit(y, X).fit().params
pval= sm.Logit(y, X).fit().pvalues
...
accuracy_score(X_test['y'],y_test)
precision_score(X_test['y'],y_test)
recall_score(X_test['y'],y_test)
f1_score(X_test['y'],y_test)
